...
Always prefer `node .claude/scripts/gemini-bridge.js` over raw `gemini` commands. The
...
```bash
node .claude/scripts/gemini-bridge.js -- "<TASK>"
```

With directories:

```bash
node .claude/scripts/gemini-bridge.js --dirs src,docs -- "<TASK>"
```

With file patterns:

```bash
node .claude/scripts/gemini-bridge.js --files "schemas/**/*.json,data/**/*.csv" -- "<TASK>"
```

With model override:

```bash
node .claude/scripts/gemini-bridge.js --model <MODEL> -- "<TASK>"
```
